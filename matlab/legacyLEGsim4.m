...
x_path = linspace(-0.0325, 0.0325, L);
...
% --- Compute motor velocities & accelerations (already quantized) ---
...
I = 0.4;       % Moment of inertia (kg*m^2)
b = 0.03;      % Damping coefficient (N*m*s/rad)

% --- Raw torque: what the motor "feels" ---
torque1_raw = I * theta1_ddot + b * theta1_dot;
torque2_raw = I * theta2_ddot + b * theta2_dot;

% --- Predict step-induced torque ---
% Model each step as producing a sharp acceleration spike
% Use moving window derivative of quantized motion
predicted_torque1 = I * theta1_ddot + b * theta1_dot;  % in this case same as raw, but can be shaped
predicted_torque2 = I * theta2_ddot + b * theta2_dot;

% --- Detrended torque: remove step prediction ---
torque1_detrended = torque1_raw - predicted_torque1;
torque2_detrended = torque2_raw - predicted_torque2;
...
% --- Plot raw and predicted torque ---
subplot(3,1,2);
plot(time, torque1_raw, 'b', time, torque2_raw, 'r'); hold on;
plot(time, predicted_torque1, '--b', time, predicted_torque2, '--r');
ylabel('Torque (Nm)');
legend('Raw M1','Raw M2','Pred M1','Pred M2');
title('Raw vs Predicted Step Torque');

% --- Plot detrended torque ---
subplot(3,1,3);
plot(time, torque1_detrended, 'b', time, torque2_detrended, 'r');
ylabel('Detrended Torque (Nm)');
xlabel('Time (s)');
legend('Motor 1','Motor 2');
title('Torque After Step-Effect Subtraction');
